...
module.exports = createCoreController(
  "api::user-contract.user-contract",
  ({ strapi }) => ({
    async draftSave(ctx) {
    ...
    async draftSavePrivate(ctx) {
      ...
      if (userId && ctx.request.body?.data) {
        ...
        if (profile && contract && contract.company_profile) {
          ...
          if (authUserContracts.length <= 0) {
            throw new ForbiddenError("You didn't say the magic word (36)");
          }
          ...
          const authUserCompanyIds = authUserContracts.map(
            (object) => object.company_profile.id
          );
          if (!authUserCompanyIds.includes(contract.company_profile)) {
            console.log(
              `auth user does not contract with the company id ${contract.company_profile}.`
            );
            throw new ForbiddenError("You didn't say the magic word (48)");
          }

          // validate setting access_role
          const authUserAccessRole = authUserContracts.find(
            (object) => object.company_profile.id === contract.company_profile
          )?.access_role;
          if (
            (contract.access_role === "owner" &&
              (authUserAccessRole === "user" ||
                authUserAccessRole === "admin")) ||
            authUserAccessRole === "user"
          ) {
            throw new ForbiddenError("You didn't say the magic word (61)");
          }

          let userProfileId;
          // check if user profile exists.
          let userProfile = await strapi.db
          ...
          if (userProfile) {
            ...
            const createUserProfile = await strapi.db
              .query("api::user-profile.user-profile")
              .create({
                select: [
                  "id",
          ...
          if (id) {
            userContract = await strapi.db
              .query("api::user-contract.user-contract")
              .findOne({
                select: ["id"],
                where: { id },
              });
            if (!userContract) {
              throw new NotFoundError("Does not exist (120)");
            }
          ...
          const userContractData = {
            ...
            date_start: contract.date_start
              ? dayjs(contract.date_start).format("YYYY-MM-DD")
              : null,
            date_end: contract.date_end
              ? dayjs(contract.date_end).format("YYYY-MM-DD")
              : null,
          ...
          if (userContractId) {
            ...
            const updateUserContract = await strapi.db
              .query("api::user-contract.user-contract")
              .update({
            ...
            const createUserContract = await strapi.db
              .query("api::user-contract.user-contract")
              .create({
                data: userContractData,
              });
    ...
    async draftDelete(ctx) {
      ...
      if (userId && ctx.params.id) {
        // retrieve contract
        const userContract = await strapi.db
          .query("api::user-contract.user-contract")
          .findOne({
            select: ["id"],
        ...
        if (!companyId) {
          throw new NotFoundError("Does not exist (220)");
        }

        const authUserContracts = await this.getAuthUserContracts(userId);
        if (authUserContracts.length <= 0) {
          throw new ForbiddenError("You didn't say the magic word (225)");
        }
        ...
        if (!authUserCompanyIds.includes(companyId)) {
          console.log(
            `auth user does not contract with the company id ${companyId}.`
          );
          throw new ForbiddenError("You didn't say the magic word (237)");
        }
        // validate setting access_role
        const authUserAccessRole = authUserContracts.find(
          (object) => object.company_profile.id === companyId
        )?.access_role;
        if (authUserAccessRole === "user") {
          throw new ForbiddenError("You didn't say the magic word (244)");
        }
      ...
      if (userId && ctx.request.body?.data) {
        ...
        if (userProfileId) {
          ...
          if (userProfile) {
            ...
            let updateUserProfile = await strapi.db
              .query("api::user-profile.user-profile")
              .update({
                select: ["id"],
                where: { id: userProfileId },
    ...
    async getAuthUserContracts(userId) {
      ...
      const authUserProfile = await strapi.db
        .query("api::user-profile.user-profile")
        .findOne({
          select: ["id"],
          where: { users: userId },
